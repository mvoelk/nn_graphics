...
\begin{tikzpicture}
    ...
    \coordinate (shapecoord) at (\layerwidth/2+0pt, -\d+4pt);
    ...
    \begin{scope} [stack]
        ...
        \node[conv] (s1) {Conv 3x3, 512}; % conv_4_3
        ...
        \node[conv] (s2) {Conv 1x1, 1024}; % fc7
        \node[conv, yshift=-\d] {Conv 1x1, 256}; % conv_6_1
        \node[conv] (s3) {Conv 3x3, 512}; % conv_6_2
        \node[conv, yshift=-\d] {Conv 1x1, 128}; % conv_7_1
        \node[conv] (s4) {Conv 3x3, 256}; % conv_7_2
        \node[conv, yshift=-\d] {Conv 1x1, 128}; % conv_8_1
        \node[conv] (s5) {Conv 3x3, 256}; % conv_8_2
        \node[conv, yshift=-\d] {Conv 1x1, 128}; % conv_9_1
        \node[conv] (s6) {Conv 3x3, 256}; % conv_9_2
    \end{scope}
    ...
    \coordinate (c3) at ($(s6) +(0pt, -2.5*\layerheight-2*\nd-2*\d)$);
    ...
    \foreach \y in {1,...,6} {
        ...
        \ifthenelse{\y=1}{
            ...
            \coordinate (c2) at ($(n1) +({(\y-1)*\layerwidth}, {-5*(\layerheight+\nd)})$);
            \begin{scope} [stack]
                \node[conv] (n5) at (c2) 
            ...
            \draw[signal] (n1.south) |- +(\d, {-4*(\layerheight+\nd)}) -| (n3.north);
        }{
